...
function App() {
  ...
  const updateTask = (taskId) => {
    Axios.put('http://localhost:3001/api/tasks/${taskId}', {
      text: updatedTask,
      priority: priority
    }).then(() => {
      setTaskList([ ...taskList, {text: updatedTask, priority: priority}]);
    }).catch((error) => {
              ...
              <>
                <div key={val.id} className={`card priority-${val.priority}`}>
                  ...
                  <div className='update'>
                    <input type='text' id='update-input' defaultValue={`${val.text}`} onChange={(e) => {
                      setUpdatedTask(e.target.value)
                    }}/>
                    <button id='update-btn' onClick={() => updateTask(val.text)}>Update Task</button>
                  </div>
                </div>
              </>
            )
